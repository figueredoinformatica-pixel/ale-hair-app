...
<div class="hero">
    ...
    <div class="hero-overlay">
        <div class="hero-title">Ale Hair</div>
        <div class="hero-sub">Premium Barber Shop em São Paulo</div>
    </div>
</div>
            """,
            unsafe_allow_html=True
...
<div style='font-size:22px; font-weight:700; margin-top:8px; color:white;'>
    {servico['Nome_Servico']}
                ...
                st.markdown(
                    """
<div style='font-size:14px; margin-top:4px; color:#D1D5DB;'>
    Serviço premium com acabamento profissional.
                ...
                st.markdown(
                    f"""
<div style='margin-top:10px; font-size:14px; font-weight:600; color:white;'>
    ⏱ {servico['Tempo_Minutos']} minutos
            ...
            with c3:
                st.markdown(
                    f"""
<div style='text-align:right; margin-top:10px; font-size:24px; font-weight:700; color:white;'>
...
# ==================================================
    ...
    st.markdown(
        f"""
<div class="resumo">
    <h2 style="margin-top:0;">{servico['Nome_Servico']}</h2>
    <p style="font-size:18px;">💰 R$ {servico['Valor_Padrao']}</p>
    <p style="font-size:16px;">⏱ {servico['Tempo_Minutos']} minutos</p>
</div>
        """,
        unsafe_allow_html=True
    )
    ...
    for i, hora in enumerate(horarios):
        ...
        with colunas[i % 4]:
            ...
            selecionado = (st.session_state.horario == hora)
    ...
    if horario:
        ...
        st.success(f"Horário selecionado: {horario}")
        ...
        if telefone:
            ...
            if len(numeros) >= 11:
                telefone = f"({numeros[:2]}) {numeros[2:7]}-{numeros[7:11]}"

        st.write("")

        if st.button("Confirmar Reserva", type="primary", width="stretch", key="confirmar_reserva"):
            ...
            if not nome or not telefone:
                ...
                st.success(
                    f"Reserva confirmada para {data.strftime('%d/%m/%Y')} às {horario}"
                )
        ...
        if st.button("Consultar", key="btn_consultar"):
            ...
            db = SessionLocal()
            resultados = buscar_agendamentos_por_telefone(db, telefone_busca)
            db.close()

            if resultados:
                st.success(f"{len(resultados)} agendamento(s) encontrado(s)")
...
# ==================================================
    ...
    st.button("⬅️ Voltar", on_click=mudar_tela, args=("catalogo",), key="voltar_login")
    st.title("🔐 Área Administrativa")

    usuario = st.text_input("Usuário", key="usuario_admin")
    senha = st.text_input("Senha", type="password", key="senha_admin")

    if st.button("Entrar", type="primary", width="stretch", key="entrar_admin"):
        login_admin(usuario, senha)
...
# ==================================================
    ...
    st.button("⬅️ Voltar", on_click=mudar_tela, args=("catalogo",), key="voltar_painel")
    st.title("📊 Agenda")
    st.button("🚪 Sair", on_click=logout_admin, key="logout_admin")
    ...
    if agendamentos:
        ...
        id_excluir = st.selectbox("Excluir agendamento", ids, key="select_excluir")

        if st.button("Excluir Agendamento", type="primary", key="btn_excluir"):
            excluir_agendamento(db, id_excluir)
